...
if (isset($_POST['save'])) {
    ...
    if (empty($errors)) {
        $checkNameStmt = $conn->prepare("SELECT category_id FROM categories WHERE name = ?");
    ...
    if (empty($errors)) {
        $checkSlugStmt = $conn->prepare("SELECT category_id FROM categories WHERE slug = ?");
                ...
                <form method="post" id="addCategoryForm" novalidate>
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <small class="text-muted">Set the category activation status</small>
                        </div>
